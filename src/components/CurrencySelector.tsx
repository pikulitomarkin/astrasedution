...
const currencies = [
  ...
  },
  {
    ...
    label: 'USD',
    name: 'Dólar',
    icon: <AstraIcon name="dollarSign" size={16} />,
  },
  {
    code: 'EUR' as const,
    label: 'EUR',
    name: 'Euro',
    icon: <AstraIcon name="euro" size={16} />,
  },
] as const;
          ...
          {currencies.map((curr) => (
            ...
            >
              <div className="flex items-center gap-2">
                <div className="w-4 h-4 flex items-center justify-center">{curr.icon}</div>
                <span className="font-medium">{curr.label}</span>
                <span className="text-xs text-zinc-500">{curr.name}</span>
              </div>
              {currency === curr.code && <div className="w-2 h-2 rounded-full bg-brand-glow" />}
            </motion.button>
          ))}
        </div>
      </div>
